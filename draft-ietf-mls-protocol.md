...
Note that while messages are not explicitly marked with the group ID
of the group they belong to, this ID is included in the
authentication of the message (as described below).  This requires
that the group ID is clear to the recipient from context, e.g.,
based on channel the message arrived on or based on the epoch.
...
The epoch fields are copied from the MLSPlaintext object directly.
...
Note that the serialization of this struct is always nine octets
...
then the whole ciphertext is used.  The first nine octets of the
...
Although this masking is itself unauthenticated, the sender data
values are authenticated both by the AEAD protection of the content
...
~~~~~
GroupOperation.confirmation = HMAC(confirmation_key,
                                   GroupState.transcript\_hash)
~~~~~
